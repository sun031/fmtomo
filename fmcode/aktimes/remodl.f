...
c     logical ldum
     ...
     1 ndex(ndp1,2),n
      ...
      data nout,tol,dtol,xtol,dmax/1,
     1 1e-6,1d-6,1.,800./
c      data delx,dpmax,drmax/2*200.,2*.01,2*75./
c      data nc/0/
*     1 2*75.,1e-6,1d-6,1.,1./
c
      call rough(50.,nk,ncr,modnam)
      a0=r(1)
c^      call assign(10,2,'remodl1.lis')
c^      write(10,200)nk,ncr,ric,roc,a0,(i,r(i),vp(i),vs(i),i=1,nk)
...
c^      write(10,201)(i,r(i),a0*z(i),(u(i,j),j=1,2),i=1,nk)
 201  format(/(1x,i5,2f10.2,2f12.6))
c^      write(10,202)delx,dpmax,drmax
...
c
      call findcp(ncr)
c^      write(10,203)(i,ucrt(i),i=1,nc)
...
c
 ...
 2    k1=k+1
c^      call retrns(10)
c^      call assign(10,2,'remodl2.lis')
c^      write(10,211)pb(1)
 211  format(/5x,'1',f12.6)
c^      write(10,212,iostat=ios)(i,pb(i),(pb(i-1)-pb(i)),i=2,n)
...
c
      call zgrid(1,mt(1))
      mm=mt(1)
c^      write(10,206,iostat=ios)(i,pm(i,1),zm(i,1),a0*(zm0(i,1)-
c^     1 zm0(i+1,1)),a0*(zm(i,1)-zm(i+1,1)),ndex(i,1),i=1,mm)
 206  format(/(1x,i5,0p2f12.6,f10.2,1pe12.4,i5))
c^      write(10,208,iostat=ios)mm+1,pm(mm+1,1),zm(mm+1,1),ndex(mm+1,1)
 208  format(1x,i5,0p2f12.6,22x,i5)
      call zgrid(2,mt(2))
      mm=mt(2)
c^      write(10,206,iostat=ios)(i,pm(i,2),zm(i,2),a0*(zm0(i,2)-
c^     1 zm0(i+1,2)),a0*(zm(i,2)-zm(i+1,2)),ndex(i,2),i=1,mm)
c^      write(10,208,iostat=ios)mm+1,pm(mm+1,2),zm(mm+1,2),ndex(mm+1,2)
c
c   Set up break pointers.
      call brkpts(mt(1),1)
c^      write(10,209)(lbrk(i,1),i=1,lbb(1))
 209  format(/(1x,20i5))
c^      write(10,210)(i,code(i,1),i=1,lcb(1))
 210  format(/(1x,i5,2x,a))
      kb(1)=lbrk(lbb(1),1)
      call brkpts(mt(2),2)
c^      write(10,209)(lbrk(i,2),i=1,lbb(2))
c^      write(10,210)(i,code(i,2),i=1,lcb(2))
      kb(2)=lbrk(lbb(2),2)
c^      write(10,*)
c^      write(10,*)'n1 kb',n1,kb
c
      ndasr  = 8*(1+2*kb(2))+4
      call dasign(nout,-2,'remodl.tbl',ndasr)
c^      write(10,*) 'reclength for dasign:', ndasr
      n1=kb(2)
      ...
      zlim=zmax
c^      write(10,*)
c^      write(10,*)'zmax zoc zic',zmax,zoc,zic
c^      write(10,*)
c
c   Loop over phases.
c
...
c^      write(10,*)'lev1',j,n,nph,sngl(pm(j,nph)),sngl(zm(j,nph))
 ...
 19   if(dabs(zmi-zoc).gt.dtol.and.dabs(zmi-zic).gt.dtol) go to 25
      ...
      pm(j,nph)=pmi
      ndex(j,nph)=nrec
c^      write(10,*)'lev2',j,n,nph,sngl(pm(j,nph)),sngl(zm(j,nph))
 ...
 27   lz=lz+1
      ...
      xl(lz,nph)=xp(n,nph)
c^      write(10,*)'lvz ',lz,n,nph,sngl(taul(lz,nph)),sngl(xl(lz,nph))
 18   continue
      j=j+1
      nrec=nrec+1
      zm(j,nph)=zmi
      pm(j,nph)=pmi
      ndex(j,nph)=nrec
c^      write(10,*)'lev3',j,n,nph,sngl(pm(j,nph)),sngl(zm(j,nph))
C^      write(10,*)
      write(nout,rec=nrec)zmi,n1,(taup(k,nph),k=1,n1),(xp(k,nph),k=1,n1)
      mt(nph)=j
      lt(nph)=lz
...
c
 ...
 30   continue
      i=mm
 31   plim=pm(i,1)
c^      write(10,*)'i zmax plim zm(i,1) =',i,zmax,plim,zm(i,1)
      do 32 i=1,mm
      if(pm(i,2).le.plim) go to 33
 32   continue
      i=mm
 33   zlim=zm(i,2)
c^      write(10,*)'i plim zlim =',i,plim,zlim
c
 17   continue
      call retrns(nout)
c
...
c^      write(10,*)'duplicate critical value eliminated:',u(k,nph)
      return
 2    continue
c
 3    nc=nc+1
...
c
      ...
      j=ic+1
      do 20 i=ic,nc 
c^      write(10,210)'i',i,ucrt(i),ucrt(j)
...
c     write(10,*)'***** i l',i,l
 19   do k=1,l
c^         write(10,*)'   i ps xs =',k,ps(k),xs(k)
      enddo
c
      x0=xs(2)-xs(1)
      do 4 k=3,l
...
c
 6    if(i.ne.ic) go to 21
      ...
      xx(n,nph)=xs(1)
      rr(n,nph)=r0
c^      write(10,201)'first ',n,pp(n,nph),xx(n,nph),rr(n,nph)
...
c^      write(10,*)'   x0 dx =',x0,dx
 ...
 14   continue
      kk=l
c^      write(10,*)'off-the-end',kk,x0,xs(kk),xs(kk-1)
 15   mm=kk
      call finrng(x0,nph,n,ps(kk),ps(kk-1),xs(kk),xs(kk-1),xtol,nr)
c^      write(10,201)'sol   ',n,pp(n,nph),xx(n,nph),rr(n,nph),nr,
c^     1 pp(n-1,nph)-pp(n,nph),a0*(xx(n,nph)-xx(n-1,nph)),rsav-rr(n,nph)
...
c^      write(10,201)' dpmax',n,pp(n,nph),xx(n,nph),rr(n,nph),nr,
c^     1 pp(n-1,nph)-pp(n,nph),a0*(xx(n,nph)-xx(n-1,nph)),rsav-rr(n,nph)
 ...
 25   if(nr.lt.nk) du=abs(pp(n-1,nph)-u(nr,nph)*(rnew/r(nr))**
     1 (alog(u(nr+1,nph)/u(nr,nph))/alog(r(nr+1)/r(nr))))
      ...
      pp(n,nph)=pp(n-1,nph)+(ps(l)-pp(n-1,nph))/ll
      xx(n,nph)=xmod(pp(n,nph),nph,1,nr,rr(n,nph))
c^      write(10,201)' drmax',n,pp(n,nph),xx(n,nph),rr(n,nph),nr,
c^     1 pp(n-1,nph)-pp(n,nph),a0*(xx(n,nph)-xx(n-1,nph)),rsav-rr(n,nph)
      k=max0(int(abs(xs(l)-xx(n,nph))/delx(nph)+.8),1)
      dx=(xs(l)-xx(n,nph))/k
      x0=xx(n,nph)
...
c
 17   continue
c^     write(10,201)'end   ',n,pp(n,nph),xx(n,nph),rr(n,nph),nr,
c^     1 pp(n-1,nph)-pp(n,nph),a0*(xx(n,nph)-xx(n-1,nph)),rr(n-1,nph)-
c^     2 rr(n,nph)
      ifl=ifl-1
...
c   Check for convergence.
 ...
 11   ps1=xi
      xs1=yi
 2    do 1 j=1,m
 1    if (abs(yi-y(j)) .GT. 1.e-6) xi=(x(j)*yi-xi*y(j))/(yi-y(j))
c 1    xi=(x(j)*yi-xi*y(j))/(yi-y(j))
      print *,'no convergence in find/remodl'
...
c
 ...
 1    continue
 2    n1=i
c^      write(10,*)'zgrid',n1,u(1,nph),sngl(pb(n1))
      l=1
      i=n1-1
      j=1
      pm(1,nph)=pb(n1)
...
c
 ...
 3    j=j-1
     ...
     1 rr(nsl1,2)
      ...
      data delx,dpmax,drmax/2*200.,2*.01,2*75./
