...
!> comment and revised by Weijia Sun. this is related to stop "more than maxpoints in raysection"
!>     if (.not.ray%valid) exit
     if (.not.ray%valid) exit     !> revised by Weijia Sun. EXIT means stop the loop.
...
!                          print '(a30,4f12.5)','refracting through top 2  ',r_interface,pt(1:3,n)
              ...
              if (iter > 10) then
                 raysec%ray%valid = .false.; deallocate(pt)
                 return
              endif
...
! dtdr dtdlat dtdlon in same case is NAN
  ...
  if (sqrt(dtdr**2+dtdlat**2+dtdlong**2) < -1.0e-10_dp) then
     print *
     print *,'----- WARNING -------'
     print *,'an interpolated time gradient was unphysically small ', sqrt(dtdr**2+dtdlat**2+dtdlong**2)
